...
from zmei_generator.config.grammar import choices, choices_or_glob
...
class TextFieldDef(DefaultTextMixin, FieldDef):
    ...
    def parse_options(self):
        ...
        field_opts = (Word(nums) | Literal('?')).setResultsName('length') + Optional(
            Suppress(',') + Suppress('choices') + Suppress(':') + choices_or_glob) + stringEnd
        ...
        if isinstance(self.options, str) and self.options.strip() != '':
            try:
                ...
                handle_parse_exception(e, self.options,
                                       'Can not parse options for field "{}" for collection "{}"'.format(self.name,
                                                                                                         self.collection.ref))
